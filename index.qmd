...
Following the established framework for context-aware Movement Analysis (CAMA) @nathan2008movement, we investigate precipitation and temperature effects (external factors) on spatial extent (navigation capacity) and on speed levels (motion capacity). It is important to note that weather as external factor can have a direct effect on motion capacity but also indirect trough navigation capacity. This is more extensively discussed in chapter 4.  

### Adapted framework from @nathan2008movement
![My Figure](fig1.png)
...
Since previous research showed the importance and sensitivity of varying scale (@laube2011, @zhao2019), a special attention is given to it in this analysis. For the weather station data, we account for the effect of spatial scale selection comparing city-level vs. station-level analysis as well as considering varying temporal scale with daily, hourly and 15min weather data. Thus, we also perform a small cross-scale movement analysis. For the mobility data, we don't include a cross-scale movement analysis, but discuss our choices and paramaters extensively.
...
In transportation planning, free flow pedestrian walking speed is assumed to be between 0.8 m/s and 1.5 m/s or 3 to 7 km/h. In our case, slower walking speeds could be possible since the city of Zurich can be rather densily packed and free flow speed might not be achieved. This is why we extended the lower threshold. 
...
To analzye the effect of precipitation on walking speed, we opted for two different spatial and temporal scales of the weathter data (small cross-scale analysis (Laube & Purves, 2011)) First, conducted an analysis with weather data on city-level. Second, we applied the same procedure, but with weather data on station-level.
...
In the following, we plot temperature vs. walking speed. As can be seen, no relationsship is identifiable. 
```{r}
#visualize
...
```
Also, the p-value with 0.7238 indicates no significance.
```{r}
# trend analysis
...
## 3. Analyze effect of precipitation with rainfall data

Next, the effect of precipitation duration on walking speed is analyzed.
...
ggplot(hourly_hulls_weather_field, aes(x = Temperature, y = log(area_m2))) +
  ...
  theme_minimal()
```
>>>>>>> c55bd02f063c71f24cf4450f16338f4b0850d095
