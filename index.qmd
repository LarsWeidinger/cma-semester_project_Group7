...
I) How does temperature affect daily travel distance, speed and sinuousity?  
II) How does precipitation affect daily travel distance, speed and sinuousity?  
...
To answer these questions we tracked ourselves with two different tracking tools. We used ArcGIS Earth, which saves a GPS fix every few seconds, independent of if you are moving or stopping, and saves it to a KML file to answer RQ I and II. We also used Google timeline, which mostly only measures your stop locations and saves it to a JSON file to answer RQ III and IV. 
...
First we split our trajectories into segments of stops and moves. Since ArcGIS Earth has a very fine sampling rates (seconds) and we sometimes let the tracking run for a whole day, a lot of the fixes are stops and will not be needed for further analysis.  
...
In the below boxplot one can see the a majority of the fixes have a speed below 20 km/h, indicating a lot of on foot travel or using public transport. 
...
In transportation planning, free flow pedestrian walking speed is assumed to be between 0.8 m/s and 1.5 m/s or 3 to 7 km/h. In our case, slower walking speeds could be possible since the city of Zurich can be rather densly packed and free flow speed might not be achieved. This is why we extended the lower threshold. 
...
Data Quality:  
-   the Google timeline data mostly records stops --> we could have switched to SDE to also model our movement in between those fixes.  
...
Concave Hulls: we only calculated it per day, even when the weather data was 1h or 15min --> this was to show that granularity matters (RQ4)
